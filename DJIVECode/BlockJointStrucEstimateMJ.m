function [Vi, curRanks, angles] = BlockJointStrucEstimateMJ(blockIn, dataname, ...
    VBars, phiBars, rBars, curRanks, outMap, theta0, optArgin, iprint, ...
    figdir)
...
%   Copyright (c)  Meilei Jiang 2018
    ...
    for ib = 1:nb
        if blockIn(ib)
            Mo2 = [Mo2, VBars{ib}];
            Qc1{ib} = eye(n, n);
            Qc2{ib} = VBars{ib} * VBars{ib}' / cosd(phiBars(ib));
        else
            ...
            % Mo1 = [Mo1, VBars{ib}]; 
            Qc1{ib} = VBars{ib} * VBars{ib}' / cosd(phiBars(ib));
            Qc2{ib} = eye(n, n);
        end
    end
    if size(Mo1, 2) == 0
    ...
    while searchNext
        ...
        Vorth = [Vorth, Vi];
        output = penaltyCCPMJ(V0(:,j), Qo1, Qo2, Qc1, Qc2, Vorth, optArgin);
